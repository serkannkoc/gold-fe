    ...
    <Stepper>
            ...
            <div class="border-2 border-dashed surface-border border-round surface-ground flex-auto flex justify-content-center align-items-center font-medium">
              ...
              >
                <template #header>
                  ...
                  >
                    <h4 class="m-0">Altin Hesabima</h4>
              ...
              >
                <template #header>
                  <div
                      class="flex flex-wrap gap-2 align-items-center justify-content-between"
                  >
                    <h4 class="m-0">Vadesiz Hesabimdan</h4>
                    <IconField icon-position="left">
                      <InputIcon>
                        <i class="pi pi-search" />
                      </InputIcon>
          ...
          <div class="flex flex-column h-12rem">
            <div class="border-2 border-dashed surface-border border-round surface-ground flex-auto flex justify-content-center align-items-center font-medium">
              <div class="field">
                <InputText
                    v-model="amount"
                    placeholder="Altin miktarini giriniz"
                />
              </div>
              </div>
          </div>
      ...
      <StepperPanel >
        <template #content="{ prevCallback, nextCallback }">
          ...
          <div class="flex pt-4 justify-content-between">
            <Button label="Back" severity="secondary" icon="pi pi-arrow-left" @click="prevCallback" />
            <Button label="Tamamla" icon="pi pi-arrow-right" iconPos="right" @click="buyGold" />
          </div>
        </template>
      </StepperPanel>
    </Stepper>
...
export default {
  ...
  methods: {
    ...
    async buyGold() {
      try {
        const response = await axios.post("http://localhost:8080/gold/buy", {
          userId: 1,
